...
def test_create_toggle_update_delete_bot_flow(tmp_path: Path):
    ...
    try:
        wallet = session.exec(
            select(WalletAddress).where(WalletAddress.bot_id == int(payload["id"]))
        ).first()
    finally:
        session.close()
    assert wallet is not None
    assert str(wallet.address) == "TRX_UPDATED"
...
def test_create_bot_record_persists_welcome_message(tmp_path: Path):
    from services.bot_service import create_bot_record
    ...
    payload = create_bot_record(
        name="Welcome Bot",
        token="welcome-bot-token-001",
        owner_name="平台自营",
        usdt_address="TRX_WELCOME",
        welcome_message="  欢迎使用我们的机器人  ",
        session_factory=session_factory,
    )
    assert payload["welcome_message"] == "  欢迎使用我们的机器人  "

    session = session_factory()
    try:
        row = session.exec(select(BotInstance).where(BotInstance.id == int(payload["id"]))).first()
    finally:
        session.close()
    assert row is not None
    assert row.welcome_message == "  欢迎使用我们的机器人  "


def test_update_bot_record_updates_welcome_message(tmp_path: Path):
    from services.bot_service import update_bot_record
    ...
    updated = update_bot_record(
        ...
        name="Platform Bot",
        owner_name="平台自营",
        usdt_address="TRX_PLATFORM",
        welcome_message="  新的欢迎词  ",
        session_factory=session_factory,
    )
    assert updated["welcome_message"] == "  新的欢迎词  "

    session = session_factory()
    try:
        row = session.exec(select(BotInstance).where(BotInstance.id == 1)).first()
    finally:
        session.close()
    assert row is not None
    assert row.welcome_message == "  新的欢迎词  "


def test_update_bot_record_empty_welcome_message_clears_field(tmp_path: Path):
    from services.bot_service import update_bot_record
    ...
    session = session_factory()
    try:
        bot = session.exec(select(BotInstance).where(BotInstance.id == 1)).first()
        assert bot is not None
        bot.welcome_message = "待清空欢迎词"
        session.add(bot)
        session.commit()
    finally:
        session.close()
    ...
    try:
        ...
        session.close()
    assert row is not None
    assert row.welcome_message is None


def test_update_bot_record_omitted_welcome_message_preserves_existing_value(tmp_path: Path):
    from services.bot_service import update_bot_record
    ...
    updated = update_bot_record(
        bot_id=1,
        name="Platform Bot Updated",
        owner_name="平台自营",
        usdt_address="TRX_PLATFORM_UPDATED",
        session_factory=session_factory,
    )
    assert updated["welcome_message"] == "保留此欢迎词"
    ...
    try:
        ...
        session.close()
    assert row is not None
    assert row.welcome_message == "保留此欢迎词"
...
def test_get_runtime_active_bot_by_token_returns_active_bot_payload(tmp_path: Path):
    ...
    session = session_factory()
    try:
        bot = session.exec(select(BotInstance).where(BotInstance.token == "platform-bot-token-001")).first()
        assert bot is not None
        bot.welcome_message = "  平台欢迎词  "
        session.add(bot)
        session.commit()
    finally:
        session.close()

    payload = get_runtime_active_bot_by_token(
        token="platform-bot-token-001",
        session_factory=session_factory,
    )
    assert payload is not None
    assert payload["id"] == 1
    assert payload["welcome_message"] == "  平台欢迎词  "


def test_get_runtime_active_bot_by_token_returns_none_for_inactive_bot(tmp_path: Path):
    from services.bot_service import get_runtime_active_bot_by_token
