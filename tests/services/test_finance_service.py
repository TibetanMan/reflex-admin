...
def test_create_manual_deposit_blocks_when_bot_wallet_missing_and_no_bot_address(tmp_path: Path):
    import pytest
...
def test_create_manual_deposit_bootstraps_wallet_from_bot_usdt_address(tmp_path: Path):
    from services.finance_service import create_manual_deposit
    ...
    try:
        wallet = session.exec(
            select(WalletAddress).where(WalletAddress.address == "TRX_FALLBACK_FIELD_ONLY")
        ).first()
    finally:
        session.close()

    assert wallet is not None
    assert str(wallet.address) == "TRX_FALLBACK_FIELD_ONLY"
