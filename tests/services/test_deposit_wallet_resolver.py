...
def test_resolve_wallet_by_bot_bootstraps_from_bot_usdt_address(tmp_path: Path):
    from services.deposit_wallet_resolver import resolve_wallet_by_bot_or_raise
    ...
    try:
        bot = BotInstance(
            token="resolver-bot-token-009",
            name="Resolver Bot Nine",
            username="resolver_bot_nine",
            status=BotStatus.ACTIVE,
            is_enabled=True,
            is_platform_bot=False,
            usdt_address="TRX_BOT_9_CONFIG",
        )
        session.add(bot)
        session.commit()
        session.refresh(bot)
        ...
        session.commit()
        assert str(wallet.address) == "TRX_BOT_9_CONFIG"
        assert int(wallet.bot_id or 0) == int(bot.id or 0)
    finally:
        session.close()
