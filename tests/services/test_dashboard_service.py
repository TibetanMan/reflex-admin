...
def test_dashboard_bot_stats_use_canonical_bot_truth_not_cached_fields(tmp_path: Path):
    from services.dashboard_service import get_dashboard_snapshot
    ...
    try:
        ...
        bot.total_users = 999
        bot.total_orders = 888
        bot.total_revenue = 777
        session.add(bot)
        session.add(
            BotUserAccount(
                user_id=int(user.id or 0),
    ...
    data = get_dashboard_snapshot(session_factory=session_factory)
    row = next(item for item in data["bot_stats"] if item["name"] == "Dashboard Bot")
