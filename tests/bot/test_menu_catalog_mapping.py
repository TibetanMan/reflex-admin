...
def test_build_purchase_delivery_file_uses_expected_filename_format():
    from datetime import datetime

    from bot.handlers.menu import _build_purchase_delivery_file

    filename, content, count = _build_purchase_delivery_file(
        library_name="全资库/一手",
        raw_items=["4111111111111111|12/30|123|US", "4222222222222222|11/29|456|US", ""],
        now=datetime(2026, 3, 13, 8, 0, 0),
    )

    assert filename == "2026-03-13-全资库_一手-2.txt"
    assert count == 2
    assert content == "4111111111111111|12/30|123|US\n4222222222222222|11/29|456|US"
