...
def test_settings_state_uses_settings_service_calls():
    ...
    create_admin_source = inspect.getsource(settings_page_module.SettingsState.create_admin_account.fn)
    ...
    assert "create_admin_account(" in create_admin_source
...
def test_inventory_state_uses_inventory_service_calls():
    ...
    append_source = inspect.getsource(InventoryState.submit_append_import.fn)
    ...
    assert "append_inventory_library_items(" in append_source
...
def test_pages_register_on_mount_loaders_for_settings_profile_inventory():
    ...
    assert "handle_inventory_page_load" in inventory_source
...
def test_settings_page_contains_admin_account_section():
    source = inspect.getsource(settings_page_module.settings)
    assert "admin_account_section" in source
